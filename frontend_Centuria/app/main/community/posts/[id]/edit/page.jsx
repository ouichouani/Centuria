...
export default function EditPost() {
    ...
    const [data, setData] = useState({ type: "Question", visibility: 'public', content: '', video: null, images: [] });
    const [previews, setPreviews] = useState([]);

    // PROBLEM : USER WANT TO DELETE ONLY ONE IMG AND LEAVE THE OTHERS IN THE POST
    // SOLUTION : THIS STATE CONTAIN THE ID OF UNWANDED IMAGES SO THE BACKEND CAN DELETE THEM 
    const [deletedImages, setDeletedImages] = useState([]);

    async function fetchPost() {

        const response = await fetch(`${domain}/posts/${id}/edit`, {
            method: "GET",
        ...
        if (result.post.content == null) result.post.content = "";
        console.log(result.post)
        setData({ ...result.post });
        if (result.post.images.length) setPreviews([...result.post.images])
    ...
    function handleChange(e) {
        ...
        if (name == 'images[]') {
            ...
            setPreviews([...previews, ...Array.from(files).map(file => ({ file, url: URL.createObjectURL(file), }))]);
            ...
            // DELETE ALL EXISTING IMAGES IF USER UPLOAD A VIDEO
            const deletedImagesArray = previews.map(item => !(item.file instanceof File) && item.id)
    ...
    async function handleSubmit(e) {
        ...
        if (data.video == null) deletedImages.forEach(id => formData.append('deletedImages[]', id));
    ...
    function removeImage(preview) {
        ...
        // FIND THE FILE OR IMAGE
        const image = previews.filter((item) => item.url == preview.url)[0];

        // THE NEW IMAGES ARRAY IN DATA STATE
        let newImagesArray = [];

        if (image.file instanceof File) {
            newImagesArray = data.images.filter(item => (item.name != image.file.name && item.lastModified != image.file.lastModified))

        } else {
            setDeletedImages(prev => ([...prev, image.id]));
            newImagesArray = data.images.filter(item => (item.id != image.id));
        }
                        ...
                        {data.video &&
                            <div className="w-full flex items-center justify-center hover:brightness-50">
                                <div className="w-fit h-fit group relative h-fit w-fit py-1 ">
                                    <video src={data?.uploadedVideo?.url ?? data?.video?.url} autoPlay controls loop className="rounded-lg max-h-[50vh]"></video>
                ...
                <div className="bg-[#151b23] absolute top-0 right-0 flex flex-col translate-x-[calc(100%+10px)] rounded-lg" >
                    ...
                    <button type="button" className='cursor-pointer flex items-center gap-0 group-hover:gap-3 rounded-lg p-2 transition-all duration-300 hover:bg-[#212830]' >
                        <label htmlFor="video" className="cursor-pointer w-[25px] h-[25px]" >
                            ...
                            <input type="file" name="video" id="video" accept="video/*" onChange={handleChange}
                                className="hidden p-2 px-3 bg-[#0d1117] border border-solid border-white/20 rounded-lg text-sm text-white file:mr-4 file:rounded-md file:border-0 file:bg-[#151b23] file:px-3 file:py-2 file:text-sm file:text-white" />
                        </label>
                    </button>
